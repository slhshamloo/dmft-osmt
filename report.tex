...
\begin{abstract}
	We investigate the Mott-Hubbard transition in the two-band Hubbard model at half filling within dynamical mean field theory. Controversy has risen regarding the nature of this transition and the existence of a single or two different critical \(U\)'s is debated. We used DMFT with a QMC impurity solver at finite temperature in the full \(J\) model. In agreement with several papers, we obtain an orbital-selective transition, with an intermediate regime where the wider band is conducting while the narrow one is insulating. The importance of the Hund coupling \(J\) is also discussed.
\end{abstract}
...
The quasiparticle weight is plotted on Fig. \ref{fig:Z_full_J}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/z_full_J_all}
...
Overall, our results are lower than those cited above. This might be agreement with theory, predicting that the critical $U$ at which transition occurs goes down as temperature is increased. Inaba et al. studied the multi-orbital model at finite temperature to study the full phase diagram, setting $J=0$ or $J=0.1U$, showing that critical temperature can decrease by a few eVs at temperature of the order of $1/40$ eV. \cite{Inaba2005}

Koga et al. argue that the isotropy in the full $J$ model is necessary to obtain an OSMT, explaining the absence of OSMT in Liebsch's results \cite{Koga2005}. In contrast, Knecht et al. \cite{Knecht2005} find lower values in the $J_z$ model, with $U_{c1} \approx 2.1$ and $U_{c2} \approx 2.6$, contradicting Liebsch's finding of the absence of OSMT \cite{Liebsch2004}. 
...
We further study the importance of the Hund coupling $J$ by running all the calculations again and setting $J = 0$ this time. The transition value is still not attained at $U = 5.5$, which is the maximum we could attain with our method before convergence issues (Fig. \ref{fig:Z_no_J}).
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/z_no_J_all}
...
We considered a two-band Hubbard model with two different bandwidths \(W_1=2\)eV and \(W_2=4\)eV at room temperature \(T=1/40\)eV. The full \(J\) model was used, and the interaction strengths fixed at \(U'=U/2\) and \(J=U/4\). The self energy and the spectral function were calculated using DMFT with a QMC impurity solver. The behaviour of the spectral weight at the Fermi level shows an orbital selective Mott transition. The system undergoes two phase transitions, first at lower \(U\) for the narrow band, and at higher \(U\) for the wide one. Between the two critical values of \(U\), we obtain an intermediate phase, where the narrow band is insulating and the wide one is still conducting, although with a very small quasiparticle weight. Another surprising phenomenon was the change of slope of the quasiparticle weight as a function of \(U\) at the transition of the narrow band. This behaviour was observed by Liebsch in the anisotropic \(J_z\) model only and by Koga in the isotropic \(J\) model only. The critical values for \(U\) are of the same order as other papers for the same model. A summarized comparison of our values compared to previous paper's results is shown in Table \ref{table_compare}.

Our findings, along with analysis of the papers published in the course of this controversy, tend to show that the multi-band Mott transition is a surprisingly complex and rich problem. Two points of view are at odds, Liebsch's that in the anisotropic \(J_z\) model there is no OSMT, and that the wide band's quasiparticle weight does not go to zero because it transitions only to a bad metal state. This is supported by Koga, who believes isotropy is necessary for an OSMT. Opposing this view is Knecht, whose results tend to show there are indeed two different phase transitions.
...
Author & T (eV)  & $U_{c1}$ (eV) & $U_{c2}$ (eV) & $U_{c1}$ (eV) & $U_{c2}$ (eV) \\
 \hline
 Liebsch, 2004 & 1/32 & 2.1 & 2.6 & 1.9 & 3.1 \\
